...
function mockMeasurement() {
	...
	Object.defineProperty(proto, 'offsetWidth', {
		...
		get: function (this: HTMLElement) {
			if (this.classList?.contains(OPTICAL_MARGIN_CLASSES.probe)) return 0
			if (this.classList?.contains(OPTICAL_MARGIN_CLASSES.word)) return WORD_WIDTH
			return CONTAINER_WIDTH
		},
	})
	Element.prototype.getBoundingClientRect = function (this: Element) {
		const el = this as HTMLElement
		if (el.classList?.contains(OPTICAL_MARGIN_CLASSES.probe)) return { width: 0 } as DOMRect
		const w = el.classList?.contains(OPTICAL_MARGIN_CLASSES.word) ? WORD_WIDTH : CONTAINER_WIDTH
